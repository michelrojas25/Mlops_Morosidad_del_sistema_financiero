...
# Función para obtener la descripción de la predicción
def get_prediction_description(prediction):
    return "paga al día" if prediction == 0 else "cliente moroso"
...
def predict(request: PredictionRequest):
    ...
    for item, prediction in zip(request.data, predictions):
        descripcion = get_prediction_description(prediction)
        cursor.execute(
            """
            INSERT INTO predicciones (input, prediccion, descripcion) VALUES (%s, %s, %s)
            """,
            (Json(item.dict()), int(prediction), descripcion)
        )
    conn.commit()
    cursor.close()
    conn.close()
